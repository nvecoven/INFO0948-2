...
*   [simxGetObjectOrientation](https://www.coppeliarobotics.com/helpFiles/en/remoteApiFunctionsMatlab.htm#simxGetObjectOrientation)
    ...
    rgbdSensor (`rgbdCasing`)

    youBot\_ref (`armRef`), youBot\_center (`ref`)
    ...
    fastHokuyo\_sensor1 (`hokuyo1`)

    youBot\_ref (`armRef`), youBot\_center (`ref`)

    (any)

    fastHokuyo\_sensor2 (`hokuyo2`)
    ...
    youBot\_gripperPositionTip (`ptip`)

    youBot\_ref (`armRef`)

    all except B.4

    youBot\_gripperPositionTarget (`ptarget`)

    youBot\_ref (`armRef`)

    all except B.4

    youBot\_gripperOrientationTip (`otip`)

    Rectangle22 (`r22`)

    all except B.4

    youBot\_gripperOrientationTarget (`otarget`)
